...
class CAllCrmActivity
{
	...
	public static function Add(&$arFields, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		// miros - new bidnings
		$create = true;
		if($arFields['BINDINGS']) {
			foreach($arFields['BINDINGS'] as $item) {
				if($item['OWNER_TYPE_ID']== \CCrmOwnerType::Contact) {
					$contact = $item['OWNER_ID'];
				}
				if($item['OWNER_TYPE_ID']==\CCrmOwnerType::Company) {
					$create = false; 
				}	
			}
		}
		if($contact && $create) {
			$cid = \CCrmContact::GetById($contact);
			if($cid['COMPANY_ID']) {
				$arFields['BINDINGS'][] = [
					'OWNER_TYPE_ID' => \CCrmOwnerType::Company,
					'OWNER_ID' => $cid['COMPANY_ID']
				];
				$arFilter = [
					'ID' => $cid['COMPANY_ID'], 
				...
				while($arEnum = $res->Fetch()) {
					if($arEnum[COMPANYUF1]) {
						$arFields['BINDINGS'][] = [
							'OWNER_TYPE_ID' => TYPE1ID,
							'OWNER_ID' => $arEnum[COMPANYUF1]
						];
	
					}
					if($arEnum[COMPANYUF2]) {
						$arFields['BINDINGS'][] = [
							'OWNER_TYPE_ID' => TYPE2ID,
							'OWNER_ID' => $arEnum[COMPANYUF2]
						];
		...
		//\Bitrix\Main\Diag\Debug::writeToFile($arFields, "activity".date("d.m.Y G.i.s"), "__debug.log");
		// end miros
